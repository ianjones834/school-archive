...
void calcArea(dimensions &, results &);
void calcPerimeter(dimensions &, results &);
...
int main()
{
  ...
  calcArea(box.sizes, box.attributes);
  calcPerimeter(box.sizes, box.attributes);
...
void calcArea(dimensions &dim, results &atr)
{
  atr.area = dim.length * dim.width;
}

void calcPerimeter(dimensions &dim, results &atr)
{
  atr.perimeter = 2 * (dim.length + dim.width);
}
