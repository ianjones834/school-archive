...
public class Course {
  ...
  public Course(String[] args) {
    ...
    try {
      ...
      numCredits = Integer.parseInt(args[6]);
    }
  ...
  public boolean dropStudentFromRoster(Student student) {
    ...
    if (isRegistered(student)) {
      ...
      this.numEnrolled--;
      student.removeCredits(numCredits);

      while (isSeatFree() && waitList.isEmpty() == false) {

        Student tmpStudent = waitList.remove(0);
        roster.add(tmpStudent);
        tmpStudent.addCredits(numCredits);
        numEnrolled++;
      }
